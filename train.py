...
tf.app.flags.DEFINE_integer(
    'BATCH_SIZE', 5, 'Batch size for training and testing')
tf.app.flags.DEFINE_float(
    'beta', 0.0005, 'Beta value for the weight decay term')
...
def _save_image_batch(epoch, batch_number, image_batch):
    ...
    save_dir = os.path.join(output_dir, "Epoch_" +
                            str(epoch), "Batch_" + str(batch_number / 10))
...
def train_neural_network():
    ...
    output, cnn_var_list, test_input, test_label, test_output = (
        model.create_model(sess, train_features, train_labels))
    ...
    (global_step, learning_rate, cnn_minimize) = model.create_optimizer(
        cnn_loss, cnn_var_list)
    ...
    for epoch in range(1, EPOCHS + 1):
        for batch in range(1, (TRAINING_DATASET_SIZE / FLAGS.BATCH_SIZE) + 1):
            ...
            if batch % CHECKPOINT_PERIOD == 0:
                ...
                feed_dict = {test_input: test_feature_batch,
